...
create the landsat cross walk dataframe
```{r}
pft_df
...
ndvi_path   = '../www/site_data/acadia/data_layers/ndvi/tera/MOD13Q1.006_250m_aid0001.nc'
...
leaflet() %>% addTiles() %>% addRasterImage(test_r, colors = pal, opacity = 0.8)
```
...
lats_merc = modis_merc_polys['9_9'][[1]]$lats
lngs_merc = modis_merc_polys['9_9'][[1]]$lngs
...
leaflet() %>% addTiles() %>% addPolygons(data = modis_wgs_polys) %>% addRasterImage(selected_pixel) %>%
  setView(lng = subset(all_site_pixels_df, row_col == '9_9')$lng_wgs_, lat = subset(all_site_pixels_df, row_col == '9_9')$lat_wgs_, zoom = 17)
...
lat_wgs = 39.0824
lng_wgs = -96.5603
albs_point = from_crs1_to_crs2_lon_lat(lon_ = lng_wgs, lat_ = lat_wgs, from_crs = wgs_crs, to_crs = alb_crs)
...
width  = 15000
height = 15000
e      = as(extent(lng_alb - width, lng_alb + width, lat_alb - height, lat_alb + height), 'SpatialPolygons')
...
e2 =as(extent(lng_merc - width, lng_merc + width, lat_merc - height, lat_merc + height), 'SpatialPolygons')
crs(e2) <- merc_crs
