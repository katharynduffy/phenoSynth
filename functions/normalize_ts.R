...
normalize_ts <- function(df, percentile = 90){
  ...
  smooth = df[,..idx]
  idx=which(colnames(df) == sprintf("smooth_ci_gcc_%s",percentile))
  ci = df[,..idx]
  upper = smooth + ci
  lower = smooth - ci
  
  # find range
  max_val = max(cbind(smooth, upper, lower), na.rm=T)
  min_val = min(cbind(smooth, upper, lower), na.rm=T)
  ...
  # return data
  d=data.frame(smooth, upper, lower)
  colnames(d)=c('smooth', 'upper', 'lower')
  return(d)
}
